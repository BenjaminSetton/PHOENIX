...
int main(int argc, char** argv)
{
	...
	// Load model
	Common::AssetHandle axeAssetID = Common::INVALID_ASSET_HANDLE;
	{
		std::shared_ptr<Common::AssetDisk> assetDisk = Common::ImportAsset("../assets/axe/scene.gltf");
		axeAssetID = ConvertAssetDiskToAssetType(assetDisk.get());
	}
	const AssetType* axeAsset = AssetManager::Get().GetAsset(axeAssetID);
	...
	phxRes = pVertexBuffer->CopyData(axeAsset->vertices.data(), vBufferSizeBytes);
	CHECK_PHX_RES(phxRes);
	...
	// Copy over asset vertex data to GPU, and keep it there permanently :)
	phxRes = pIndexBuffer->CopyData(axeAsset->indices.data(), iBufferSizeBytes);
	CHECK_PHX_RES(phxRes);
	...
	TransformData transform = InitializeTransform(initialCameraPos, fov, aspectRatio, 0.005f);
	...
	while(!pWindow->ShouldClose())
	{
		...
		pRenderPass->SetExecuteCallback([&](IDeviceContext* pContext, IPipeline* pPipeline) 
		{
			...
			pContext->CopyDataToBuffer(pVertexBuffer, axeAsset->vertices.data(), vBufferSizeBytes);
			pContext->CopyDataToBuffer(pIndexBuffer, axeAsset->indices.data(), iBufferSizeBytes);
