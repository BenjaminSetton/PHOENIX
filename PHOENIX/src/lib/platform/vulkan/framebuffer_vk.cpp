...
namespace PHX
{
	...
	FramebufferVk::FramebufferVk(RenderDeviceVk* pRenderDevice, const FramebufferCreateInfo& createInfo)
	{
		...
		m_renderPassDesc = BuildRenderPassDescription(createInfo);

		VkFramebufferCreateInfo framebufferInfo{};
		framebufferInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
		framebufferInfo.renderPass = RenderPassCache::Get().Insert(pRenderDevice, m_renderPassDesc);
	...
	ITexture* FramebufferVk::GetAttachment(u32 index)
	{
	...
	const RenderPassDescription& FramebufferVk::GetRenderPassDescription() const
	{
