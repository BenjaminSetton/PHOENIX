...
namespace Common
{
	...
	public:
		...
		void UploadStaticData(PHX::RenderGraphHandle renderGraph, PHX::SwapChainHandle swapChain);

		// Upload instance data to GPU. Call when instance data changes.
		void UploadInstances(PHX::RenderGraphHandle renderGraph, PHX::SwapChainHandle swapChain);

		// Zero the args and count buffers. Must be called before CullAndSelectGPU each frame.
		void ZeroDrawBuffers(PHX::RenderGraphHandle renderGraph, PHX::SwapChainHandle swapChain);

		// GPU cull + LOD select. Call inside a compute render graph pass callback.
		// Writes draw commands into the args buffer.
		void CullAndSelectGPU(PHX::DeviceContextHandle deviceContext,
