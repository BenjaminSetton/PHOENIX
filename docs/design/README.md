...
- Must issue the correct graphics API calls once a graphics API is selected by the client
**IRenderGraph**
- Has a high-level overview of all the render passes that have been created by the client in a given frame
- Controls all the resources of render passes. Usually the render graph will own the resources used by render passes, barring some important exceptions such as the swap chain backbuffers and texture inputs loaded from disk, and therefore it's in charge of allocating their memory. The reason it allocates it's own memory is because it allows for certain memory optimizations where GPU memory is shared between resources during a frame
- The render graph is the main component of interaction between the client and the PHX lib. This is because the render graph provides a callback for render passes which give the client access to a device context to record their GPU commands. The reason this functionality is provided as a callback is because it may or may not be called, depending on whether the render pass gets trimmed by the render graph
- During baking, this system will perform several important steps: transition resource layouts to their correct states (Vulkan only), synchronize the execution of the render passes using barriers and semaphores (Vulkan only), and finally optimize the execution of render passes by combining compatible passes and trimming unused/superfluous passes
**IRenderPass**
- Provides an interface for adding resource inputs and outputs for a render pass
- Render pass objects are obtained directly from the render graph by adding a new pass
...
Another important aspect of this system is that it also owns most of the resources used for the render passes, except for special cases such as texture inputs read from disk (e.g. PBR textures) and the swapchain backbuffers. Most of the textures used outside of the special cases are only used to somehow contribute to the final image presented in a frame. For example, the depth buffer is constructed every frame and used to implement effects such as shadow mapping. Unless the depth buffer is also contributing to some accumulated depth buffer for effects like motion blur, the render graph can allocate the memory by itself whenever necessary. This allows resources to be reused easily between passes through aliasing.