...
namespace PHX
{
	...
	STATUS_CODE CompileShader(const ShaderSourceData& srcData, CompiledShader& out_result)
	{
		...
		glslang::TShader shader(SHADER_GLSLANG_UTILS::ConvertShaderStage(srcData.stage));
		...
		glslang::TIntermediate& intermediateRef = *(program.getIntermediate(SHADER_GLSLANG_UTILS::ConvertShaderStage(srcData.stage)));
