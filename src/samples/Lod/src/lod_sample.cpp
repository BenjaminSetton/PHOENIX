...
void LodSample::InitSample()
{
	...
	m_lodManager.UploadStaticData(m_renderGraph);
}
...
void LodSample::Draw()
{
	...
	if (m_instancesDirty)
	{
		...
		m_lodManager.UploadInstances(m_renderGraph);
		m_instancesDirty = false;
	}

	// Zero draw args/count buffers before the compute pass
	m_lodManager.ZeroDrawBuffers(m_renderGraph);
	...
	// COMPUTE PASS — LOD cull + select
	{
		...
		computePass.SetExecuteCallback([this](PHX::DeviceContextHandle deviceContext)
		{
			...
			m_lodManager.CullAndSelectGPU(deviceContext, m_computeUniforms, camData);
		});
	}
